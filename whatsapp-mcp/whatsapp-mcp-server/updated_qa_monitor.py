#!/usr/bin/env python3
"""
Updated Google Sheets QA Monitor - Uses group chat messaging
=====================================================================

Monitors the Google Sheets tabs for when the "Incomplete"
checkbox (column V) is ticked and triggers QA feedback communication
to the appropriate WhatsApp group chats.
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('updated_qa_monitor.log'),
...
# Support multiple sheet tabs - now includes Lawley as well
SHEET_NAMES = ["Velo Test", "Mohadin WA_Tool Monitor", "Lawley WA_Tool Monitor"]

# State tracking
MONITOR_STATE_FILE = 'updated_qa_monitor_state.json'
...
def trigger_qa_feedback(drop_data: Dict, dry_run: bool = False) -> bool:
    ...
    try:
        ...
        # Determine project from sheet name - map sheet names to project names
        sheet_name = drop_data.get('sheet_name', 'Velo Test')
        project_mapping = {
            'Velo Test': 'Velo Test',
            'Mohadin WA_Tool Monitor': 'Mohadin',
            'Lawley WA_Tool Monitor': 'Lawley'
        }
        project_name = project_mapping.get(sheet_name, sheet_name)

        # Create feedback message
        message = create_feedback_message(drop_number, missing_steps, project_name, user)

        # Send feedback to the appropriate group chat using the updated function
...
def monitor_sheets_for_incomplete(dry_run: bool = False, check_interval: int = 60):
    ...
    logger.info("🚀 Starting UPDATED Google Sheets QA Monitor...")
    ...
    logger.info(f"📱 Sending feedback to GROUP CHATS (not private numbers)")
    logger.info("=" * 70)
                ...
                for row_index, row in enumerate(sheet_data):
                    ...
                    if is_incomplete:
                        ...
                        if row_key not in processed_incomplete:
                            logger.info(f"🚨 NEW INCOMPLETE: {drop_number} (Row {row_index + 1}) in {sheet_name}")
                            ...
                            if drop_number not in feedback_sent_history:
                                ...
                                if trigger_qa_feedback(parsed_row, dry_run):
                                    ...
                                    logger.info(f"✅ Feedback sent and recorded for {drop_number} in {sheet_name}")
                                else:
                                    logger.error(f"❌ Failed to send feedback for {drop_number} in {sheet_name}")
                            else:
    ...
    logger.info("🛑 Updated Google Sheets QA Monitor stopped")

def main():
    # Logger is already initialized at module level
    setup_logging()  # For compatibility, but logger is already set

    parser = argparse.ArgumentParser(description='Updated Google Sheets QA Monitor for Incomplete Flags')
    ...
    try:
        ...
        if args.test:
            ...
            if sheet_data:
                logger.info(f"✅ Successfully read data from sheets")
                # Process first few rows as test from each sheet
                for sheet_name, values in sheet_data.items():
                    logger.info(f"📊 Testing {sheet_name} sheet:")
                    for i in range(min(5, len(values))):
                        parsed = parse_sheet_row(values[i], i + 1)
                        if parsed:
