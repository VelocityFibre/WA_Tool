#!/usr/bin/env python3
"""
Mohadin Isolated Service
Dedicated service for monitoring Mohadin WhatsApp group with parallel testing mode.
"""
...
class MohadinService:
    """
    Mohadin-specific service wrapper.
    Handles Mohadin group monitoring with safe parallel testing configuration.
    """
    
    def __init__(self):
        self.service_id = "mohadin"
        self.port = 8081
    ...
    def _setup_logging(self) -> logging.Logger:
        """Setup Mohadin-specific logging."""
        logger = logging.getLogger('MohadinService')
    ...
    def validate_configuration(self) -> bool:
        """Validate Mohadin-specific configuration."""
        config = self.service.config
        
        # Check required Mohadin configuration
        required_fields = [
        ...
        if not config.get('whatsapp', {}).get('parallel_testing_mode', False):
            self.logger.warning("⚠️  Parallel testing mode should be enabled for Mohadin")
    ...
    def print_startup_info(self):
        """Print startup information for Mohadin service."""
        config = self.service.config
        
        print("🎯 MOHADIN SERVICE STARTING")
        print("=" * 50)
    ...
    def start(self):
        ...
        try:
            ...
            self.logger.info("🚀 Starting Mohadin monitoring service...")
            self.service.start()
            
        except KeyboardInterrupt:
            self.logger.info("🛑 Mohadin service interrupted by user")
        except Exception as e:
            self.logger.error(f"❌ Mohadin service failed: {e}")
            sys.exit(1)

def main():
    """Main entry point for Mohadin service."""
    print("🎯 Mohadin WhatsApp Group Monitoring Service")
    print("⚠️  PARALLEL TESTING MODE - Safe monitoring with zero production impact")
    print()
    
    mohadin_service = MohadinService()
    mohadin_service.start()
